...
## 2026-09-05 — Fronteira de autorização do Studio OS: route group + wrapper + guard de CI

**Decisão:** a autorização do admin (Epic 2) é estrutural e verificável por máquina, em três camadas. (1) **Leituras**: toda página/layout/route vive sob `app/admin/(protected)/`, cujo `layout.tsx` roda `getCurrentUser()` + `hasMinimumRole(user.role, "staff")` antes de renderizar `children`. (2) **Escritas**: toda Server Action é criada por `defineAdminAction({ role, input? }, handler)` (`lib/auth/admin-action.ts`), que resolve o usuário, aplica `requireRole` retornando `ActionResult` (`{ ok: false, error }` em pt-BR) em vez de lançar, valida input com Zod opcional (`fieldErrors` no fracasso), e captura throws do handler em erro genérico logado — o handler nunca roda sem `CurrentUser` autorizado. (3) **Guard de CI**: `scripts/check-admin-auth.mjs` faz walk estático em `app/admin/` e falha o build se um arquivo `"use server"` não chama `defineAdminAction(`, ou se um `page`/`layout`/`route` está fora de `(protected)/`. Rodado por `npm run check:admin-auth`, plugado no `ci.yml` após `npm run lint`.

**Motivo:** com a RLS hoje inerte no caminho da aplicação (decisão de 2026-09-03 sobre RBAC como fronteira autoritativa), a autorização depende inteiramente de a camada de aplicação chamar `requireRole` em todo lugar certo — uma disciplina que revisão humana esquece. Amarrar o guard ao layout do route group (uma página nova herda o guard só por estar no diretório certo) e ao wrapper (uma action nova é rejeitada se não for construída pelo wrapper), e então falhar o CI quando qualquer um dos dois é contornado, torna o esquecimento um erro de build em vez de um furo silencioso.

**Isenção:** `app/admin/login/` é o ponto de entrada não autenticado — o sign-in não tem `CurrentUser` para atravessar `defineAdminAction`, e a página renderiza antes de qualquer papel existir. É isento das duas regras do guard (o brief original só isentava `login/` da regra de página/route; estendido para a regra `"use server"` porque `app/admin/login/actions.ts` a violaria de outra forma).

**Alternativas consideradas:** (a) só o guard do layout, sem o wrapper — deixa Server Actions descobertas, que são o vetor de escrita; (b) só o wrapper, sem o guard de CI — não pega uma página nova criada fora de `(protected)/`; (c) middleware/`proxy.ts` fazendo a checagem de papel por match de path — frágil a mudança de rota e já rejeitado como mecanismo primário na decisão de 2026-09-03.

## 2026-09-05 — Admin UI: primitivas escritas à mão sobre tokens do Tailwind v4 (não shadcn/ui)

**Decisão:** as primitivas de UI do Studio OS (`components/ui/`: `Field`, `Input`, `Textarea`, `Select` nativo, `Card`, `Badge`, `DataTable`, `Modal` sobre `<dialog>`, `PageHeader`, `EmptyState`) são componentes pequenos e sem estado, escritos à mão, um por arquivo, consumindo os design tokens editoriais do protótipo V2.3 já em `app/globals.css` via classes utilitárias do Tailwind v4 (`border-line`, `bg-champ`, `text-muted`, `font-serif`). O único helper compartilhado é `cn()` (`lib/cn.ts`), um `classes.filter(Boolean).join(" ")` — sem `clsx`, sem `tailwind-merge`. Novo token `--danger` / `--color-danger` (`#b3261e`) adicionado a `globals.css` para estados de erro.

**Motivo:** o `docs/reference` (stack proposta) cita shadcn/ui, mas shadcn assume Tailwind v3 com `tailwind.config.ts` e traz Radix como dependência para cada primitiva. Este projeto já decidiu Tailwind v4 CSS-first sem arquivo de config (decisão de 2026-09-03), e as telas do Studio OS precisam de um punhado de primitivas simples cujo visual tem que casar exatamente com os tokens do protótipo já validado. Escrever ~15 linhas por componente é menos custo do que adaptar o gerador do shadcn ao setup v4 e revisar o CSS gerado contra os tokens. `Modal` usa o elemento nativo `<dialog>` (`showModal()`/`close()`) em vez de um portal + trap de foco à mão.

**Consequência:** as tasks seguintes do Epic 2 constroem sobre essas primitivas e não devem introduzir shadcn/ui nem uma lib de componentes. Uma primitiva nova segue o mesmo padrão: um arquivo em `components/ui/`, sem estado quando possível, tokens via utilitárias, `cn()` para merge de `className`. Se um componente precisar de `clsx`/`tailwind-merge` de verdade (conflito real de classes Tailwind), essa é a hora de reavaliar `cn()` — não antes.

## 2026-09-06 — Timezone handling: `datetime-local` → instante UTC na fronteira do formulário

**Decisão:** todo `<input type="datetime-local">` do admin converte o valor para um instante absoluto no client, com `new Date(local).toISOString()`, antes de enviá-lo à Server Action. O browser interpreta a string sem offset ("2026-10-18T14:30") como hora local da máquina, e `toISOString()` devolve o instante correspondente já com offset explícito (`...Z`) — exatamente o que `z.iso.datetime({ offset: true })` (`createPaymentSchema.paidAt`) exige. A alternativa que estava no código, concatenar `"Z"` na string local, rotulava a hora de parede como UTC e deslocava o registro financeiro pelo offset do browser (até algumas horas), inclusive jogando o pagamento no mês errado do extrato.

**Motivo:** `payments.paid_at` é `timestamp with time zone`; um instante ambíguo ali não é recuperável depois, e `getFinancialLedger` agrupa por mês a partir dele.

**Ponto áspero conhecido:** os intervalos de mês do dashboard e do extrato (`domain/dashboard/queries.ts`, `domain/finance/ledger.ts`) são calculados em UTC. Enquanto o estúdio operar em um único fuso e não registrar pagamentos na virada da meia-noite, a diferença é invisível; se isso mudar (ou se o estúdio passar a operar através de uma fronteira de data), o cálculo de intervalo precisa passar a ser feito no fuso do estúdio, não em UTC. Revisitar aí, não antes.

## 2026-09-06 — Epic 2 (Studio OS) encerrado

**Decisão / estado:** as 14 tasks do Epic 2 (`SCL-200`–`SCL-240`) estão `DONE` em `main`, cada uma revisada individualmente (spec + qualidade), mais uma revisão final de branch inteiro (opus) que gerou 0 Critical / 8 Important + minors, todos corrigidos numa única leva e re-revisados (verdict: pronto para merge). Nenhuma migration nova neste epic — o schema do Epic 1 permaneceu congelado.

**O que foi construído:** shell do admin + `defineAdminAction` (wrapper de RBAC) + `scripts/check-admin-auth.mjs` (guard de CI); dashboard com KPIs derivados; CRM de clientes (lista/cadastro/ficha/edição); agenda (lista + filtros); criação de ensaio confirmado ponta a ponta (`shoot` + `production_job` + checklist inicial numa `db.transaction`); ficha do ensaio; registrar pagamento (insert + recálculo do cache `shoots.payment_status` na mesma transação, com `FOR UPDATE` na linha do shoot); livro-caixa + KPIs financeiros; despesas; Kanban de produção + transições de status guardadas (com reflexão de `finalizado`/`entregue` no ensaio); `changeShootStatusAction` para avançar o status do ensaio pelo pipeline; checklist de preparação interno.

**Invariantes verificadas na revisão final (independentemente do guard):** (1) nenhuma escrita ao banco fora de `domain/*/service.ts` e dos módulos de transação — nenhum Server Component muta nada; (2) `shoots.payment_status` escrito em exatamente um lugar (`domain/payments/register-payment.ts`, dentro da transação); (3) todo KPI/extrato/saldo/progresso é recalculado na leitura, nada persistido como agregado; (4) dinheiro é string decimal ponta a ponta, aritmética só em `@/lib/money` + `domain/payments/balance.ts`.
...
**Styling:** Supabase Storage foi escolhido para este marco, no bucket privado `styling-references`, com URLs assinadas temporárias. O limite é 8 MiB por JPEG/PNG/WebP e 20 ocupações por Shoot, contando a união deduplicada de rows e objetos. Paths são canônicos em três segmentos. A fix wave adiciona `0029_styling_upload_reservations.sql`, `0030_guard_styling_row_deletion.sql` e `0031_serialize_styling_lifecycle.sql`: metadata reserva a cota antes do upload; Storage exige a row correspondente; a exclusão confirma primeiro a ausência do objeto; e upload, delete de metadata e nova reserva compartilham o mesmo advisory transaction lock por Shoot. Assim, objetos órfãos históricos continuam consumindo cota e o teto permanece serializável sob concorrência. As migrations anteriores `0024`–`0028` não foram reescritas.
